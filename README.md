...
- **Пересылка вместо добавления в чат**: объявление можно просто переслать боту в личку — оно пройдёт тот же каскад (правила → ИИ) и встанет в модерацию; пересылки дедуплицируются (канальный пост не создаст дубль, даже если бот уже есть в канале).
