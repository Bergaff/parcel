...
- **Только посылки**: пассажирские попутки («Пассажир. Гродно-Минск», «кто подвезёт до…») бот пропускает — в личке вежливо объясняет, что доска про посылки; объявления водителей публикуются.
- **SEO-страницы маршрутов**: `/r/варшава-львов` и ещё 46 популярных направлений — текст под запросы «передать посылку Варшава Львов» + живые заявки; каталог `/routes`; sitemap.xml динамический (главная + маршруты + активные объявления).
