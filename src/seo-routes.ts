...
function howtoText(r: SeoRoute, seed: string): Array<[string, string]> {
  return pick([
    [
      ['Найдите заявку', `Откройте список ниже или доску с фильтром «${r.from} → ${r.to}». В каждой заявке видно дату выезда, сколько килограмм готов взять водитель и контакт.`],
    ...
    [
      ['Разместите просьбу', `Если подходящего рейса сейчас нет, опишите, что и когда нужно передать по маршруту ${r.from} → ${r.to} — объявление бесплатное и появится на доске после проверки модератором.`],
