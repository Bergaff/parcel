...
const CITY_FORMS: Record<string, string> = {
  ...
  // Бяла-Подляска на линии Варшава — Брест: в чатах пишут и «Бяла», и «Белая»,
  // и латиницей. Один город — одно написание, иначе поиск и дедупликация
  // считают их разными маршрутами.
  'бяла': 'Бяла', 'бялой': 'Бяла', 'бялу': 'Бяла', 'бяла-подляска': 'Бяла',
  'белая': 'Бяла', 'белой': 'Бяла', 'белу': 'Бяла',
  'biala': 'Бяла', 'biała': 'Бяла', 'biala podlaska': 'Бяла', 'biała podlaska': 'Бяла',
  'тересполь': 'Тересполь', 'тересполя': 'Тересполь', 'тересполе': 'Тересполь', 'terespol': 'Тересполь',
  'седлеце': 'Седлеце', 'седльце': 'Седлеце', 'siedlce': 'Седлеце',
  ...
  'москва': 'Москва',
  // У Москвы и других городов на «-а» окончание меняет последнюю букву
  // (Москв-ы, Москв-у), а не добавляет её — парсер это не ловил, и
  // «из Москвы в Краков» оставалось без маршрута. Плюс частое «мск».
  'москвы': 'Москва', 'москву': 'Москва', 'москве': 'Москва', 'мск': 'Москва',
  'риги': 'Рига', 'ригу': 'Рига', 'риге': 'Рига',
  'праги': 'Прага', 'прагу': 'Прага', 'праге': 'Прага',
  'вены': 'Вена', 'вену': 'Вена', 'вене': 'Вена',
  'одессы': 'Одесса', 'одессу': 'Одесса', 'одессе': 'Одесса',
  'санкт-петербург': 'Санкт-Петербург', 'питер': 'Санкт-Петербург', 'спб': 'Санкт-Петербург',
...
const SEPARATORS = /^(?:\s*(?:->|=>|>>|→|⇒|↔\uFE0F?|—|–|−|-|,|;|до|в|на|из|с|от|через)\s*(?:[а-яёa-z]{0,12}\s*)?)$/;
...
type CityHit = { city: string; index: number; end: number };

/** Соседние ли города в цепочке: между ними только разделитель. */
function chained(text: string, a: CityHit, b: CityHit): boolean {
  if (b.index < a.end) return false; // наложение форм одного и того же слова
  const between = text.slice(a.end, b.index).replace(/\uFE0F/g, '');
  if (between.length > 40) return false;
  const clean = between.trim();
  return clean === '' || SEPARATORS.test(clean);
}

/**
 * Извлекает маршрут из текста. Водители пишут цепочками:
 * «Белосток-Кузница-Гродно», «Катовице, Варшава, Брест, Минск»,
 * «Минск Брузги Варшава». Маршрут — от первого города к последнему:
 * Кузница, Брест и Брузги по пути, но едет человек из Белостока в Гродно.
 * Раньше брали первую подходящую пару, и «Белосток-Кузница-Гродно»
 * превращался в «Белосток → Кузница».
 */
function extractRoute(text: string): { from: string; to: string } | null {
  // Один и тот же город ловится дважды (ключи «варшава» и «варшав»+окончание
  // совпадают в одной точке) — оставляем первое совпадение, иначе дубль рвёт цепочку
  const cities: CityHit[] = [];
  for (const c of findCities(text)) {
    const last = cities[cities.length - 1];
    if (last && c.index < last.end) continue;
    cities.push(c);
  }
  // Самая длинная цепочка подряд идущих разделённых городов
  let best: CityHit[] = [];
  let run: CityHit[] = [];
  for (const cur of cities) {
    if (run.length === 0 || chained(text, run[run.length - 1]!, cur)) {
      run.push(cur);
    } else {
  ...
  if (best.length >= 2) {
    const from = best[0]!;
    const to = best[best.length - 1]!;
    // Кольцо «Минск - Варшава - Минск»: конец тот же город — берём первую
    // пару, направление всё равно уточнят в переписке
    if (from.city === to.city) return { from: from.city, to: best[1]!.city };
    return { from: from.city, to: to.city };
  }
  return null;
}
...
const PASSENGER_HINTS = [
  ...
  /ищ[уе]\s+попутк/i,
  // «Ищу 1 место, рюкзак», «Ищу 3 места Минск-Варшава (аэропорт Модлин)» —
  // это поиск места в машине, а не передача посылки. У водителя «есть места»
  // паттерн не матчится (нужно «ищу/ищем»).
  /ищ[уем]\s*(?:\d+\s*)?мест/i,
];
